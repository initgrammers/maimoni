...
- `hono-wide-logger`: ^0.1.1 - Wide-event logging middleware for Hono

### Type Dependencies
- `hono-wide-logger` includes TypeScript types via module augmentation
...
The wide-logger middleware is mounted before auth middleware to capture full request lifecycles:
...
After successful authentication, the user ID is attached to the logger context:
...
Route handlers add business context using the helper function:
...
### Business Context Usage Example
...
With business context, logs can be queried by:
...
- **2026-02-24**: Business Context Enhancement archived from change `hono-wide-logger-business-context`
  - Added `addBusinessContext()` helper function to all 8 route files
  - Standardized business context fields: endpoint, entityType, action, boardId, entityId
  - Updated AGENTS.md with logging pattern documentation
  - All 132 tests passing
  
- **2026-02-23**: Initial specification archived from change `hono-wide-logger-integration`
  - Implemented structured logging with hono-wide-logger
  - Integrated with factory-pattern auth middleware
  - Replaced console.error in scan routes
  - All 132 tests passing

### Requirement: Business Context Helper Function

The system MUST provide a type-safe helper function `addBusinessContext()` in `apps/api/src/routes/types.ts` that allows route handlers to add business context to the wide-logger.

#### Scenario: Helper function adds context to logger (happy path)
...
#### Scenario: Helper handles missing logger gracefully (edge case)
...
### Requirement: Business Context Fields

The business context MUST include the following fields:

- **endpoint** (required): snake_case identifier describing the operation (e.g., 'create_expense', 'update_board_settings')
- **entityType** (optional): Type of business entity being operated on
  - Valid values: 'board', 'expense', 'income', 'category', 'invitation', 'scan', 'auth'
- **action** (optional): Type of operation being performed
  - Valid values: 'create', 'update', 'delete', 'list', 'get', 'scan', 'claim'
- **boardId** (optional): UUID of the board when operating within a board context
- **entityId** (optional): UUID of the specific entity (expenseId, incomeId, invitationId, etc.)

#### Scenario: Business context logged for board operations (happy path)
...
#### Scenario: Business context logged for entity operations (happy path)
...
### Requirement: Route Business Context Integration

The system MUST add business context logging to all API route files:

- `dashboard.ts`: GET /dashboard endpoint
- `boards.ts`: PATCH /boards/:boardId/settings, DELETE /boards/:boardId
- `expenses.ts`: All 5 expense endpoints (POST, GET, PATCH, DELETE, list)
- `incomes.ts`: All 5 income endpoints
- `categories.ts`: GET /categories endpoint
- `scan.ts`: POST /scan endpoint
- `invitations.ts`: All 6 invitation endpoints
- `auth.ts`: POST /auth/claim endpoint

#### Scenario: All routes log consistent business context (happy path)

- GIVEN any API request to an endpoint with business context integration
- WHEN the route handler executes
- THEN the log SHALL include consistent business context fields
- AND the naming convention SHALL follow `{action}_{entity}` snake_case pattern
- AND PII SHALL NOT be included in business context (only IDs)

## References

- Package: https://www.npmjs.com/package/hono-wide-logger
- Change Archive: `openspec/changes/archive/2026-02-23-hono-wide-logger-integration/`
- Change Archive: `openspec/changes/archive/2026-02-24-hono-wide-logger-business-context/`
